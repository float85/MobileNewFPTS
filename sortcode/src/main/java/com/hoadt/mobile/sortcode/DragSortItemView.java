package com.hoadt.mobile.sortcode;
...
public class DragSortItemView extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (child instanceof LinearLayout)
        {
          ...
          ((LinearLayout) child).setWeightSum(2.0f);
        }
        ...
        for(int i = 0; i<((ViewGroup)child).getChildCount(); ++i) {
            ...
            if (i==0)
            {
                if (nextChild instanceof CheckedTextView)
                {
                    ...
                    ((CheckedTextView) nextChild).setGravity(Gravity.CENTER);

                    //((Checkable) nextChild).setGravity(Gravity.CENTER_VERTICAL);
                    //((Checkable) nextChild).setChecked(true);
                }
            }
            if (i==1)
            {
